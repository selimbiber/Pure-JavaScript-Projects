const NEWSLETTER_CARD = document.getElementById('newsletter-card')
const FIGURE_SECTION = document.getElementById("card_figure-section");
const FIGURE_IMG = document.getElementById("card_figure-img");
...
SUBMIT_INPUT.addEventListener("click", (e) => {
	...
	if (EMAIL_INPUT_LENGTH == 0) {
        ...
        if (window.innerWidth >= 1440) {
            NEWSLETTER_CARD.setAttribute('class', 'newsletter-card_success-styles')
        }
	}
});
...
DISMISS_BTN.addEventListener("click", () => {
    ...
    if (window.innerWidth >= 1440) {
        NEWSLETTER_CARD.setAttribute('class', 'newsletter-card_default-styles')
    }
});

function isEmailValid(email) {
	const regex =
		/^[^\s@]+@[^\s@]+\.[^\s@]+$/; /* Expected Email Value Structure = "ash@loremcompany.com" */
